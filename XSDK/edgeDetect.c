...
XTime tStart,tbeforeSobel,tafterSobel, tEnd;
...
static char srcImg[] = "loro.bmp";
static char dstImg[] = "sloro.bmp";
...
int main()
{
	...
	if (Status != XST_SUCCESS) {
		xil_printf("Error en la ejecución de Sobel\r\n");
		return XST_FAILURE;
	}
	...
	printf("Tiempo Total %.2f us, Tiempo Sobel %.2f us\r\n",1.0*(tEnd-tStart)/(COUNTS_PER_SECOND/1000000),1.0*(tafterSobel-tbeforeSobel)/(COUNTS_PER_SECOND/1000000));
